...
for ziel1 in zielList:
  ...
  #7  n=27;runde=0;phase=0;w,h=1000,1000;x0,y0=501,501;w0,h0,xAlt,yAlt=0,0,x0,y0;feldList=[];first=True;wMax=w;hMax=h
    ...
    listX = erzeugeList(w0,y0,w,y0+1)
    while True:
        ...
        if phase == 0:
            ...
            if phase == 1:
                x0,y0,w0,h0,w,h,xAlt,yAlt,listY = sucheList(bomb_dir,x0,y0,xAlt,yAlt,w0,h0,w,h,listY,first);first=False;laenge=len(listY)
            else:
        ...
        if len(listX) < 150 and phase == 0:
            phase = 1;first=True
            listY = erzeugeList(x0,h0,x0+1,h)
        if len(listY) < 150 and phase == 1:
            phase = 2;first=False
            if len(listX) == 0 or len(listY) == 0:
                feldList = erzeugeList(w0,h0,w,h)
            else:
                feldList = newList(listX,listY)
