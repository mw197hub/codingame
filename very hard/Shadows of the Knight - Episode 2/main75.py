...
def sprungX(bomb_dir,x0,y0,xAlt,yAlt,w0,h0,w,h,first):
    ...
    if bomb_dir == "WARMER":
        if x0 > xAlt:
            w0=xAlt+int((x0-xAlt) /2)-1 if xAlt >= w0 else w0  # w0+int((x0-xAlt) /5)-1
        else:
            w=xAlt-int((xAlt-x0)/2)+1
        if w-x0 > x0-w0:
            x1=w-int((w-x0) /3)+1
        else:
            x1=w-int((x0-w0)/3)-1
    ...
    if bomb_dir == "COLDER":
        if x0 > xAlt:
            w=x0-int((x0-xAlt) /2)+1 
        else:
            w0=x0+int((xAlt-x0)/2)-1 if x0 >= w0 else w0  # w0+int((xAlt-x0)/4)-1
         #   w = xAlt if w > xAlt else w
        if xAlt - w0 > w - xAlt:
            x1=w0+int((xAlt-w0) /3)-1
        else:
            x1=w0-int((w-xAlt) /3)+1
        return x1,y1,w0,h0,w,h,x0,y0
...
#3 zielList.clear();zielList=['0-50']

for ziel1 in zielList:
  #3  n=12;runde = 0;phase=0;w,h=1,100;x0,y0=0,0;w0,h0,xAlt,yAlt=0,0,x0,y0;feldList=[];first=False
    n=27;runde=0;phase=0;w,h=1000,1000;x0,y0=501,501;w0,h0,xAlt,yAlt=0,0,x0,y0;feldList=[];first=False
    ...
    while True:
        ...
        if w - w0 < 300 and phase == 0:
            phase = 1;first=True
        if h - h0 < 300 and phase == 1:
            phase = 2
