...
ANZAHLTROLLE=5;RUNDETROLL=200;MAXTROLLWERT=3;OPPCHOP=False
...
def setChopDistanz(treeDict,xyPos,moveZiel,zielDict,minWert,scoreList):    
    distanzDict={}
    for treeId,tree in treeDict.items():
        if not treeId in moveZiel:
          ...
          #  if tree.fruits > 0 or (tree.size == 4 and dist > tree.cooldown):
            if tree.type in ['BANANA']:
                dist = dist - 2
            distanzDict[treeId] = dist
    return distanzDict    
def setTreeDistanz(treeDict,xyPos,moveZiel,zielDict,minWert,scoreList):    
    distanzDict={};fehltWas=[];vorhandenList=[]
    for wert,anzahl in scoreList.items():
        if wert in ['plum','lemon','apple']:
            if anzahl > minWert:
                vorhandenList.append(wert.upper())
            else:
                fehltWas.append(wert.upper())
    for treeId,tree in treeDict.items():
        if not treeId in moveZiel:
            dist = getDistanz(xyPos,setXY(tree.x,tree.y))
            if tree.fruits > 0 or (tree.size == 4 and dist > tree.cooldown):
                if tree.type in fehltWas:
                    dist = dist - 6
                distanzDict[treeId] = dist
    return distanzDict
        ...
        for tree,dist in treeDist.items():
            if dist > 2 and not tree in warenList:
                for trollId,ziel in zielDict.items():
                    if ziel.ziel == "" and (not ziel.name == "PICK" and not ziel.name=="HOLE") and trollDict[trollId].waren == 0 and getDistanz(trollDict[trollId].xy,myBase) == 1:
...
def bewegung(width,height,runde,myBase,oppBase,lineList,scoreDict,treeDict,trollDict,ergList,grundList,moveList,graph,ironList,waterList,baseDict,zielDict):
    ...
    for trollId,troll in trollDict.items():
        ...
        if troll.player == 0:
            ...
            if troll.speed + troll.capacity > ironWert and len(zielDict[trollId].ziel) == 0:
    ...
    for trollId,troll in trollDict.items():
        if troll.player == 0:
            ...
            #elif troll.xy in treeList and (runde > RUNDETROLL - 80 or treeDict[troll.xy].type == "BANANA") and troll.chop <= treeDict[troll.xy].size:                
            elif len(trollList) >= ANZAHLTROLLE:
                if troll.xy in treeList:
                    ergList.append(["CHOP",str(troll.id)])
                else:
                    troll.treeDict=setChopDistanz(treeDict,troll.xy,moveZiel,zielDict,len(trollList)+1,scoreList)
            ...
            #elif troll.xy in treeList and treeDict[troll.xy].fruits > 0 and scoreList[treeDict[troll.xy].type.lower()] < 26:                 
            elif troll.xy in treeList and treeDict[troll.xy].fruits > 0:                
                ergList.append(["HARVEST",str(troll.id)])
            elif troll.capacity > troll.waren: # suche waren
                troll.treeDict=setTreeDistanz(treeDict,troll.xy,moveZiel,zielDict,len(trollList)+1,scoreList)
                if len(troll.treeDict) > 0:
                    zielXY = troll.getShortTree()
                else:
                    ## was dann
    ...
    if len(trollList) < ANZAHLTROLLE and runde < RUNDETROLL:
        ...
        if scoreList['plum'] >= minWert and scoreList['lemon'] >= minWert and scoreList['apple'] >= minWert and scoreList['iron'] >= minWert:
            eisen=0
            for trollId,troll in trollDict.items():
                eisen+=troll.iron
            if eisen == 0:
                ...
                if lemon >= MAXTROLLWERT:
                    lemon=MAXTROLLWERT
                if apple >= MAXTROLLWERT-1:
                    apple=MAXTROLLWERT-1                    
