...
class Agent:
    def __init__(self,id,x,y,owner,shoot_cooldown,optimal_range,soaking_power,cooldown,splash_bombs,wetness,moeglicheZiele):
        ...
        self.moeglicheZiele=moeglicheZiele
        self.xy=str(x)+"-"+str(y)
...
def setReihenfolge(feldDict,agentDict,feldBombDict,my_player,wallDict,enemyList,myList,enemyTrefferDict,tempVerteidigungList):
    reiheDict={};tempMyList=myList[:]
    for myId in myList:        
        mZiele=agentDict[myId].moeglicheZiele
        wert=len(mZiele)*10
...
runde,my_player=initMap(feldDict,moveList,wallDict,agentGrundDict,graph,agentDict,myList,enemyList,verteidigungList)


if runde == 1 and len(verteidigungList) == 0:
    setVerteidigung(feldDict,agentDict,feldBombDict,my_player,wallDict,enemyList,myList,verteidigungList)
...
setReihenfolge(feldDict,agentDict,feldBombDict,my_player,wallDict,enemyList,myList,enemyTrefferDict,tempVerteidigungList)
...
#Deckung suchen
        ...
        #if not agent.xy in tempVerteidigungList and runde < 8:
        #    distN=999
        #    for feld in tempVerteidigungList:
        #        weg = A_path(graph,agent.xy,feld)
        #        distanz = len(weg)
        #        if distanz < distN and weg[1] in tempMoveList:
        #            move = feld;distN=distanz
        #    if len(move) > 0:
        #        tempVerteidigungList.remove(move)
