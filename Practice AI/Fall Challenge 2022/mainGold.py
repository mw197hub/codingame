...
def setZielList(width, height,feldDict,enemyDict,myDict,recDict,myFeldDict,enemyFeldDict,neutralFeldDict):
    myMitteX,myMitteY,enemyMitteX,enemyMitteY=0,0,0,0
    yList=[]
    for xy in myDict:
        x,y=getXY(xy);myMitteX+=x;myMitteY+=y
    for xy in enemyDict:
        x,y=getXY(xy);enemyMitteX+=x;enemyMitteY+=y
    links=False if myMitteX > enemyMitteX else True
    myMitteX= int(myMitteX/4) +1 if links else int(myMitteX/4) -1
    distY = (abs(myMitteY-enemyMitteY) / 4) -2
    for xy in myDict:
        x,y=getXY(xy) 
        if x == myMitteX:
            yList.append(y)
    ...
    if links:
        x=int((width-1)/2) if width / 2 == width //2 else int((width-1)/2)
      #  x=int(width/2) if width / 2 == width //2 else int((width+1)/2)
    else:
        x=int(width/2) if width / 2 == width //2 else int((width-1)/2)
    ...
    if not 0 in yList:
        yList.append(0)
    if not height -1 in yList:
        yList.append(height-1)
    for y in range(height):
        if not y in yList:
            yList.append(y)    
    schritt = int(height / 2) if height / 2 == height //2 else int(height/2 +1) - distY
    for y in yList:
        mod = 0
        if abs(y - yList[0]) >= schritt:
            mod = abs(y - yList[0]) // schritt
            if y > yList[0]:
                if links:
                    mod = mod * -1
            else:
                if links:
                    mod = mod * -1
        xy = setXY(x+int(mod),y)
        if xy in feldDict:
            zielList.append(xy)
    startPhase = x + 1 + int(width//3)
    return zielList,startPhase,links
...
def getShortDist(myFeldDict,neutralFeldDict,feldDict,myDict,enemyFeldDict,bereichList,action,runde):
    ...
    for xy in myFeldDict:        
        ...
        #for xyE in feldDict[xy].moeglich:
            for xyE in myDict[xy].moeglich:
                xE,yE=getXY(xyE)
                if abs(x-xE)+abs(y-yE) < dist and not xy == xyE and (xyE in neutralFeldDict or xyE in enemyFeldDict):
        ...
        if not xZ == -1:
            setMove(my,my.units,x,y,xZ,yZ,action,feldDict,myFeldDict,runde)
        elif not xZ2 == -1:
            setMove(my,my.units,x,y,xZ2,yZ2,action,feldDict,myFeldDict,runde)

def getSpawnDist(myFeldDict,enemyDict,feldDict,myDict,bereicheList,action,links,my_matter,neutralFeldDict,buildList):    
    for bereich in sorted(bereicheList, key=len):
        if bereich[0] > 0 and bereich[1] > 0:
            wertDict={}
            for b in bereich[3:]: 
                if b in myFeldDict and not b in buildList:
...
def setMove(f1,anz,xS,yS,xZ,yZ,action,feldDict,myFeldDict,runde):
    f1.units -= anz
    diffX=xZ-xS;diffY=yZ-yS
    yN = yS -1 if diffY < 0 else yS + 1
    if abs(diffY) >= 1 and setXY(xS,yN) in f1.move and runde < 12:    # bevorzugt oben und unten
    ...
    if my_matter >= 10:
        for bereich in bereicheList:
            if bereich[1] > bereich[0]:
                for b in bereich[3:]:
                    feld = feldDict[b]
                    if feld.can_build and feld.amount_get > 18 and feld.enemy_near > 0 and not b in buildList:
                        action.append("BUILD {} {}".format(feld.x,feld.y))
...
# move
        ...
        for ziel in zielList:
            ...
            if len(xyS) > 0:
                ...
                if myFeldDict[xyS].units > 1 and len(zielList) > anz:
                    setMove(myFeldDict[xyS],1,x,y,xZ,yZ,action,feldDict,myFeldDict,runde);anz+=1
                else:
                    setMove(myFeldDict[xyS],1,x,y,xZ,yZ,action,feldDict,myFeldDict,runde);anz+=1
    ...
    if runde > startPhase or len(myMoveList) > 0:
        for xy in myMoveList:
            my = myDict[xy]
            if my.units > 0:
                getShortDist([xy],neutralFeldDict,feldDict,myDict,enemyFeldDict,bereichList,action,runde)
# spawn
    if my_matter >= 10:
        getSpawnDist(myFeldDict,enemyDict,feldDict,myDict,bereicheList,action,links,my_matter,neutralFeldDict,buildList)
