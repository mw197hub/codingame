...
for i in range(1,n+1):
    zahlList.append(i)
ergList=[];endList=[]
erg="";zahl=1;verschoben=False;anzahl=0
while(True):
    ...
    for i in iList:
        ...
        if zahl2 == zahl:
            if zahl2 == 21 or zahl1 == 21:
                a=0
            pos1=suchePos(zahlList,zahl1)
            pos2=suchePos(zahlList,zahl2)
            if pos2 < pos1:
                verschoben=True
                zahlList.remove(zahl)
                nList=[];erledigt=False
                for i1 in range(len(zahlList)):
                    if not erledigt and i1 >= pos1 and (i1 == len(zahlList)  or zahlList[i1] > zahl):
                        nList.append(zahl2)
                        erledigt=True
                    nList.append(zahlList[i1])
                if not erledigt:
                    nList.append(zahl2)
                zahlList = copy.deepcopy(nList)
                if len(zahlList) <20:
                    a=0

    zahl+=1
    if verschoben:
        zahl=1;verschoben=False;anzahl+=1
        if anzahl > n*3:
            erg="INVALID"
    if zahl > n or len(erg) > 0:
        break
