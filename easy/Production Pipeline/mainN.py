...
def suchePos(suchList,zahl):
    for i in range(len(suchList)):
        if suchList[i] == zahl:
            return i
    return -1

def einsetzen(zahlList,suchList,zahl,zahlB):
    pos = suchePos(suchList,zahlB)
...
for i in iList:
    ...
    if zahl1 == 3 or zahl2 == 3:
        a=0
    #if zahl1 in zahlList:
    #    zahlList.remove(zahl1)
    #if zahl2 in zahlList:
        #zahlList.remove(zahl2)
    erledigt1=-1;erledigt2=-1
    ...
    if erledigt1 == -1 and erledigt2 == -1:       
        zahlList.remove(zahl1);zahlList.remove(zahl2)
        ergList.append([zahl1,zahl2])
    elif erledigt1 > -1 and erledigt2 > -1:
        if erledigt1 == erledigt2:
            pos1 =suchePos(ergList[erledigt1],zahl1)
            pos2 = suchePos(ergList[erledigt1],zahl2)
           # if pos1 > pos2:
           #    erg="INVALID"
        else:            
            if erledigt1 > erledigt2:
                iList1 = ergList.pop(erledigt1);iList2= ergList.pop(erledigt2)
            else:
                iList2= ergList.pop(erledigt2);iList1 = ergList.pop(erledigt1)
            pos = suchePos(iList1,zahl1)
            nList=[]
            for i1 in range(len(iList1)+len(iList2)):
                if len(iList1) == 0:
                    nList.append(iList2.pop(0))
                elif len(iList2) == 0:
                    nList.append(iList1.pop(0))    
                elif iList1[0] == zahl1 and iList2[0] == zahl2:
                    nList.append(iList1.pop(0))
                elif iList1[0] < iList2[0]:
                    nList.append(iList1.pop(0))
                else:
                    nList.append(iList2.pop(0))
            ergList.append(copy.deepcopy(nList))

    elif erledigt1 > -1:
        iList1 = ergList.pop(erledigt1)
        ergList.append(einsetzen(zahlList,iList1,zahl2,zahl1))
    else:
        iList2 = ergList.pop(erledigt2)
        ergList.append(einsetzen(zahlList,iList2,zahl1,zahl2))


for i1 in range(n-len(zahlList)):
    kleinste=n+99;tabInd=-1
    for i2 in range(len(ergList)):
        if len(ergList[i2]) > 0:
            eList = ergList[i2]
            if eList[0] < kleinste:
                kleinste=eList[0];tabInd=i2
    endList.append(ergList[tabInd].pop(0))


if len(erg) > 0:
    print("INVALID")
else:
    for zahl in zahlList:  
        nList=[]    
        for e in endList:
            if zahl < e:
                nList.append(zahl);zahl=99999999
            nList.append(e)
        if zahl < 99999999:
            nList.append(zahl)
        endList = copy.deepcopy(nList)
    for e in endList:
        erg=erg+str(e)+" "
    print(erg[:-1])
