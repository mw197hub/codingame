...
def erstelleGraph(aktFloor,aktPos,aktDirection,eList,bList,elevatorDict,nb_floors, width, nb_rounds, exit_floor, exit_pos, nb_total_clones, nb_additional_elevators, nb_elevators,zwangEleDict):
    ...
    if 0 in elevatorDict:
        gesamtList.append(["WAIT"])
        gesamtList.append(["BLOCK"])
    else:
        ...
        if i in zwangEleDict:
          ...
          #  if i in elevatorDict and len(elevatorDict[i] > 1):
            for gList in ersatzList:
                gList1=gList[:];gList1.append("WAIT")
                gesamtList.append(gList1)
                gList2=gList[:];gList2.append("BLOCK")
                gesamtList.append(gList2)
    ...
    for gList in gesamtList:
        ...
        newList=[]
        if anzFrei > 0:
            combiList = list(combinations(range(exit_floor+lastLevel-1),anzFrei))
           # print(combiList,file=sys.stderr)
            for combi in combiList:
                nList=[]
                for i in range(len(gList)):
                    if i in combi and not gList[i] == "ELEVATOR" and not gList[i] == "ZWANG":
                        nList.append("ELEVATOR")
                    else:
                        nList.append(gList[i])
                if not nList in doppeltList:
                    newList.append(nList[:])
                    doppeltList.append(nList[:])
        else:
            newList.append(gList)
        
        for combi in newList:
            if combi == ['ELEVATOR', 'BLOCK', 'WAIT', 'ELEVATOR', 'ELEVATOR', 'BLOCK', 'WAIT', 'WAIT', 'WAIT', 'BLOCK', 'ZWANG']: #8
            #if combi == ['ELEVATOR', 'WAIT', 'WAIT', 'BLOCK', 'WAIT', 'WAIT', 'BLOCK', 'ELEVATOR', 'WAIT', 'ZWANG', 'ZWANG']: #10
                print(combi)

            eList,bList,runde,fertig = erstelleListen(aktFloor,aktPos,aktDirection,eList,bList,elevatorDict,nb_floors, width, nb_rounds, exit_floor, exit_pos, nb_total_clones, nb_additional_elevators, nb_elevators,zwangEleDict,combi,deathDict)
